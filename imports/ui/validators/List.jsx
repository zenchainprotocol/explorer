...
import { Badge, Progress, Row, Col, Card, Spinner, Button } from 'reactstrap';
...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <SentryBoundary><Row className="validator-info">
            ...
            {(!props.inactive) ? <Col className="uptime data" xs={{ size: 2, }} md={2}><Progress animated value={props.validator.uptime}><span className="d-none d-md-inline">{props.validator.uptime ? props.validator.uptime.toFixed(2) : 0}%</span><span className="d-md-none">&nbsp;</span></Progress></Col> : ''}
            {(!props.inactive) ? <Col className="delegate data" xs={{ size: 2, order: "last" }} md={2}><Button  color="primary" size="sm">Delegate</Button></Col> : ''}
